...
#calcul indice de rareté des espèces
  ...
  Indice_rarete_sp=NULL
  site_poisson<-matrix(0,26,length(recensement_unique$Code_CT_SC_M))

  for (n in 1:length(b)){
  site_poisson[,n]<-c(as.vector(recensement_unique$Code_Site[recensement_unique$Code_CT_SC_M==b[n]]),rep(0,26-length(recensement_unique$Code_Site[recensement_unique$Code_CT_SC_M==b[n]])))
  names(site_poisson[,n])=b[n]
  ...
  sites.etudies=targeted.stations[targeted.stations!=suppressed.station]
    ...
    poissons_dans_station<-matrix(0,59,26)
    Indice_rarete_station<-NULL
    recensement_unique$Code_Site
