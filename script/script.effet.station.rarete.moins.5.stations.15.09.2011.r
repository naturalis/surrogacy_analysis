...
#calcul indice de rareté des espèces
  ...
  Indice_rarete_sp=NULL
  site_poisson<-matrix(0,22,length(recensement_unique$Code_poisson))

  for (n in 1:length(b)){
  site_poisson[,n]<-c(as.vector(recensement_unique$Code_Site[recensement_unique$Code_poisson==b[n]]),rep(0,22-length(recensement_unique$Code_Site[recensement_unique$Code_poisson==b[n]])))
  names(site_poisson[,n])=b[n]
  ...
  sites.etudies<-targeted.stations[!targeted.stations%in%suppressed.station]
    ...
    poissons_dans_station<-matrix(0,335,22)
    Indice_rarete_station<-NULL
    recensement_unique$Code_Site
